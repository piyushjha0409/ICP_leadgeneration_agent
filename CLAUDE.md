...
- **qualify** — ONE batched strategist call scoring all candidates (batching forces ranking; isolated scoring yields "a pile of 80s"); explicit 40/30/30 fit/pain/timing rubric, with `scoreReasons` requested as `Fit: <pts> — …` / `Pain:` / `Timing:` lines (the leads page parses those prefixes); disqualification is structural-only. The evaluation schema is structural only — no `.min(1)` on reasons, no score bounds — because the shared output contract permits empty arrays and a strict schema aborted whole batches live; empty reasons and out-of-range scores are repaired in code and reported as `withoutReasons`
